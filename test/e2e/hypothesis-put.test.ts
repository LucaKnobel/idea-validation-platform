...
describe('PUT /api/hypotheses/:hypothesisId integration', async () => {
  ...
  it('updates an owned hypothesis and persists the new statement and canvas links', async () => {
    ...
    const response = await updateHypothesisWithCookie(user.cookieHeader, existingHypothesis.id, {
      statement: '  Updated statement  ',
      dimension: 'MARKET',
      priority: 'HIGH',
      evidenceType: 'QUALITATIVE',
      canvasElementTypes: ['VALUE_PROPOSITIONS', 'VALUE_PROPOSITIONS']
    })
    ...
    expect(payload.canvasSections).toHaveLength(1)
    expect(payload.canvasSections[0]).toBe('VALUE_PROPOSITIONS')
  ...
  it('rejects invalid route params with 400', async () => {
    ...
    const response = await updateHypothesisWithCookie(user.cookieHeader, 'still-not-a-uuid', {
      statement: 'Updated statement',
      dimension: 'MARKET',
      priority: 'HIGH',
      evidenceType: 'QUALITATIVE',
      canvasElementTypes: ['VALUE_PROPOSITIONS']
    })
  ...
  it('returns 404 when another user tries to update the hypothesis', async () => {
    ...
    const response = await updateHypothesisWithCookie(attacker.cookieHeader, existingHypothesis.id, {
