...
describe('DELETE /api/ideas/:id/versions/:versionId/hypotheses/:hypothesisId integration', async () => {
  ...
  it('deletes an owned hypothesis and removes it from the database', async () => {
    ...
    const response = await deleteHypothesisWithCookie(user.cookieHeader, createdVersion.ideaId, createdVersion.ideaVersionId, hypothesis.id)
  ...
  it('rejects invalid route params with 400', async () => {
    ...
    const response = await deleteHypothesisWithCookie(user.cookieHeader, 'not-a-uuid', 'also-not-a-uuid', 'still-not-a-uuid')
  ...
  it('returns 404 when another user tries to delete the hypothesis', async () => {
    ...
    const response = await deleteHypothesisWithCookie(attacker.cookieHeader, createdVersion.ideaId, createdVersion.ideaVersionId, hypothesis.id)
